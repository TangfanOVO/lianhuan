...
| 服务 | 干什么 | 注意 |
|---|---|---|
| 端到端全双工语音（如火山「豆包」的 duplex realtime dialogue） | 一条 WebSocket 里同时做识别和发声 —— 通话的主力路子 | ★ **key 绝不能进前端**，必须自己架一条中继：浏览器 ←WS→ 你的服务 ←WS→ 供应商 |
| 语音合成 / 换声音 | 让它用某个音色说话 | 同上，key 留服务端 |
...
（0830 清零：原先「来源没核清」的两件都查实了 —— 字云出自 ThreeUI（MIT，已内置），
水面待机是**本项目纯原创**（设计 handoff 原件在项目自己手里），也内置了。）
