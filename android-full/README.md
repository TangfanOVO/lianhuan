...
> - 仓库里**没配固定签名钥匙**的时候，Actions 打的是 debug 包，每次的签名都不一样，
>   于是新包**装不到旧包上面**，想更新只能先卸载 —— 那正是会丢数据的那条路。
>
> 所以在配好固定签名之前，**把它当一次性试用包**：
> 想留下什么，先去 设置 › 搬家 导出一份 JSON。
>
> 配好固定签名之后（下面有步骤），新包直接覆盖安装，数据留着，这一段就不用管了。
